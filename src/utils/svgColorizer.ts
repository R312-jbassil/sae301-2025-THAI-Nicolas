/**
 * Utilitaire pour colorier les SVG de lunettes
 * Applique les couleurs personnalisées aux différentes parties du SVG
 */

export interface LunettesColors {
  monture: string;
  branches: string;
  verres: string;
}
...
/**
 * Applique les couleurs aux éléments SVG des lunettes
 * @param svg - L'élément SVG à colorier
 * @param couleurs - Les couleurs à appliquer (monture, branches, verres)
 */
export function applySVGColors(
  svg: SVGElement,
  couleurs: LunettesColors
): void {
...
/**
 * Retourne le contenu SVG complet des lunettes
 * @param viewBox - ViewBox personnalisé (optionnel)
 * @returns Le code HTML du SVG
 */
export function getSVGContent(viewBox: string = "0 0 595.28 841.89"): string {
  return `
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="${viewBox}" class="w-full h-auto">
      <defs>
        <style>
